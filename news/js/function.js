...
// btn选项卡按钮 con选项卡内容
	...
	function xuanxiangka1(btn1,con1){
		const btn=$(btn1);
		const con=$(con1);
			for(let i=0;i<btn.length;i++){//var不是块级作用域，所以如果用var会覆盖
				btn[i].onmouseover=function(){
...
function jielunbo(bigbannerBox,picbox,pic,left,right,num,lunboTime,donghuaTime){
   ...
   // $(function(){})
	...
	// donghuaTime  动画时间
	// num  导航出现的高度
	function louceng(anniu,Section,anniukuai,daohang,chushise,donghuaTime,num,colorArr,dingbu){
		const btn=$(anniu);
		const section=$(Section);
		const btnbox=$(anniukuai)[0];
		const nav=$(daohang)[0];
		const fanhui=$(dingbu)[0];
	...
	// 添加cookie   addCookie(key,val,[time]);
	// key:键名   string
	// val:键值   string
	// time过期时间 可填可不填 单位是秒
